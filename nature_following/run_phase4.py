...
def battery_thermal_ode(t, y, power_profile, actions):
    ...
    target_flows = total_flow * valves / max(1e-3, valves.sum())
    dflowdt = (target_flows - flows) / 2.0
...
def run_phase4_training():
    print("Starting Phase 4: PPO Infrastructure Verification...")
    power_profile, speed_profile = load_original_drive_cycles()
    
    # Use a truncated horizon for rapid infrastructure verification
    EVAL_HORIZON = 500
    TRAIN_EPOCHS = 10
    ...
    for name, agent in rl_agents.items():
        ...
        for ep in range(TRAIN_EPOCHS):
            ...
            for t in range(EVAL_HORIZON):
                ...
                # Simulate step using Euler for speed during infrastructure verification
            ...
            train_logs.append({
                'model': name, 'epoch': ep, 'reward': total_r, 
                'actor_loss': a_loss, 'critic_loss': c_loss, 'entropy': ent
            })
            
    df_logs = pd.DataFrame(train_logs)
    df_logs.to_csv("metrics/training_log.csv", index=False)
