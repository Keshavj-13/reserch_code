...
class PPOAgent:
    ...
    def select_action(self, xs, adj, xt, xg):
        with torch.no_grad():
            mu, std, val, z = self.policy(xs, adj, xt, xg)
        return mu.squeeze(0).numpy(), val.item()
...
class PIDStandard(BaselineController):
    def get_action(self, state, t): 
        val = 0.5 if state.max() > 30.0 else 0.0
        return np.full(NUM_ZONES + 2, val)
        
class PIDAdaptive(BaselineController):
    def get_action(self, state, t):
        val = np.clip((state.max() - 28.0) / 10.0, 0, 1)
        return np.full(NUM_ZONES + 2, val)
        
class UniformFlow(BaselineController):
    def get_action(self, state, t): return np.full(NUM_ZONES + 2, 1.0)
    
class ProportionalTemp(BaselineController):
    def get_action(self, state, t):
        val = np.clip((state.max() - INITIAL_TEMP) / (TEMP_MAX - INITIAL_TEMP), 0, 1)
        return np.full(NUM_ZONES + 2, val)
        
class MPCHorizon(BaselineController):
    def get_action(self, state, t):
        return np.random.uniform(0, 1, NUM_ZONES+2) # Dummy fast MPC for infrastructure test

baselines = {
    'PID_Standard': PIDStandard(), 
    'PID_Adaptive': PIDAdaptive(), 
    'Uniform_Flow': UniformFlow(), 
    'Proportional_Temp': ProportionalTemp(), 
    'MPC_H1_S32': MPCHorizon()
}

def run_phase6_benchmark():
    print("Starting Phase 6: Full Controller Benchmark...")
    ...
    for name, c in {**baselines, **rl_agents}.items():
        ...
        for t in range(horizon):
            if name in rl_agents:
                ...
                a_t, _ = c.select_action(xs, adj, xt, xg)
                a = np.clip(a_t, 0.0, 1.0)
            else:
                a = c.get_action(state, t)
