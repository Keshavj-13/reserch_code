...
The implemented recovered training branch uses fixed reward weights throughout each training run.
These fixed weights were selected to keep safety and spatial uniformity dominant while retaining penalties for energy use and abrupt valve changes.
The chosen weights balance safety, efficiency, uniformity, and smoothness outcomes in the recovered comparison campaign.

\begin{table}[!tb]
\centering
\caption{Reward weights used in the recovered training branch. Values are taken from `tags/recovered_physics_v1/core_physics_v1.py`.}\label{tab:reward_weights}
\begin{tabular}{lll}
\hline
...
\(\lambda\) terms & Folded into fixed weights & No separate runtime schedule\\
\hline
\end{tabular}
\end{table}
...
Training uses full episode on policy rollouts with generalised advantage estimation throughout.
Regularisation through a fixed entropy bonus controls premature policy collapse.

\begin{table}[!tb]
\centering
\caption{PPO training hyperparameters used in the recovered training branch. Values are traced to `run_phase_g_train.py`.}\label{tab:ppo_hyperparameters}
\begin{tabular}{ll}
\hline
...
Training initialises the log standard deviation to \(-1.0\) for each action dimension.
Wide distributions encourage broad exploration of the continuous valve and actuator command space.
As training progresses, the learned standard deviations can decrease as the policy finds good commands.
